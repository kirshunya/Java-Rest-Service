...
public class BankServiceImpl implements BankService {
  ...
  public void deleteBank(Long id) {
    Bank bank =
        repository.findById(id).orElseThrow(() -> new EntityNotFoundException(BANK_NOT_FOUND + id));
  ...
  public Bank addUserById(Long usId, Long bkId) {
    User user =
        userRepository.findById(usId).orElseThrow(() -> new RuntimeException("User not found"));
  ...
  public Bank addUser(User user, Long bkId) {
    Bank bank =
        repository
            .findById(bkId)
            .orElseThrow(() -> new RuntimeException(BANK_NOT_FOUND + "with id: " + bkId));
    bank.getUsers().add(user);
