...
public class UserServiceImpl implements UserService {
  ...
  public User findUserById(Long id) {
    return repository
        .findById(id)
        .orElseThrow(() -> new RuntimeException(USER_NOT_FOUND + "with id: " + id));
  }
  ...
  public PayCard updateCard(Long usId, PayCard card, Long cardId) {
    User user = findUserById(usId);
    PayCard cardToUpdate =
        user.getPayCards().stream()
  ...
  public String deleteCard(Long usId, Long cardId) {
    User user =
        repository.findById(usId).orElseThrow(() -> new EntityNotFoundException(USER_NOT_FOUND));
    PayCard payCard =
        user.getPayCards().stream()
  ...
  public User addBankById(Long usId, Long bkId) {
    User user = repository.findById(usId).orElseThrow(() -> new RuntimeException(USER_NOT_FOUND));
    Bank bank =
        bankRepository.findById(bkId).orElseThrow(() -> new RuntimeException("Bank not found"));
    user.getBanks().add(bank);
